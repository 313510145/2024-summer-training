...
public:
    ...
    int** const load_gray(const std::string& file_name, int* const w, int* const h) const;

    // Load an RGB image from the given file and return a 3D array of pixel values
    static int*** const load_RGB(const std::string& file_name, int* const w, int* const h);

    // Save a grayscale image to a file using pixel data and dimensions
    static const void dump_gray(const int& w, const int& h, int** const pixel, const std::string& file_name);

    // Save an RGB image to a file using pixel data and dimensions
    static const void dump_RGB(const int& w, const int& h, int*** const pixel, const std::string& file_name);

    // Display a grayscale image in an X server window
    static const void display_gray_X_server(const int& w, const int& h, int** const pixel);

    // Display an RGB image in an X server window
    static const void display_RGB_X_server(const int& w, const int& h, int*** const pixel);

    // Display a grayscale image as ASCII characters in the terminal
    static const void display_gray_ASCII(const int& w, const int& h, int** const pixel);

    // Display an RGB image as ASCII characters in the terminal
    static const void display_RGB_ASCII(const int& w, const int& h, int*** const pixel);

    // Display an image using a command line tool
    static const void display_command_line(const std::string& file_name);

    // List all files in a directory and store the file names in a vector
    static const bool list_directory(const std::string& directory_path, std::vector<std::string>& file_name);
...
private:
    ...
    static const bool file_exist(const std::string& file_name);
